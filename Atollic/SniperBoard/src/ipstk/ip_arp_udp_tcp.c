...
// packetloop_icmp_tcp(buf,enc424j600PacketReceive(BUFFER_SIZE, buf));
...
void send_wol(uint8_t *buf,uint8_t *wolmac)
{
        ...
        enc424j600PacketSendP1(UDP_HEADER_LEN+IP_HEADER_LEN+ETH_HEADER_LEN+102,buf);
}
#endif // WOL_client
...
void client_set_gwmac(uint8_t *mac)
{
	uint8_t i;
	i=0;
	while(i<6){
